...
회고
 Pomodoro 타이머 구현을 통해 몇 가지 중요한 점을 배웠습니다:
 	.	모듈화의 중요성: 각 기능(작업 세션, 짧은 휴식, 긴 휴식)을 별도의 메서드로 분리함으로써 코드의 가독성과 유지보수성을 높였습니다. 이는 향후 기능 확장이나 수정 시 유용할 것입니다.
 	.	Timer.periodic의 활용: Dart의 `Timer.periodic`을 사용하여 주기적인 작업을 효과적으로 구현할 수 있었습니다. 이는 실시간으로 변화하는 값을 다룰 때 매우 유용한 도구입니다.
 	.	사용자 경험 고려: 남은 시간을 실시간으로 출력함으로써 사용자에게 현재 상태를 명확히 전달할 수 있었습니다. 이는 실제 애플리케이션에서 중요한 UX 요소입니다.
 	.	확장성: 현재는 콘솔 기반이지만, 이 구조를 바탕으로 GUI나 모바일 앱으로 쉽게 확장할 수 있습니다. Flutter와 같은 프레임워크를 사용하여 시각적 요소를 추가할 수 있을 것입니다.
 	.	테스트의 필요성: 실제 사용을 위해서는 각 기능에 대한 단위 테스트와 통합 테스트가 필요할 것입니다. 이는 코드의 안정성을 보장하는 데 중요합니다.
 	.	사용자 설정 옵션: 향후 개선 사항으로, 사용자가 작업 시간과 휴식 시간을 직접 설정할 수 있는 기능을 추가하면 더욱 유용한 애플리케이션이 될 것입니다.
